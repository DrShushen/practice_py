...
Run this app with `python app15.py`
"""
...
def get_figure(_df, x_col, y_col, selectedpoints, selectedpoints_local):
    ...
    if selectedpoints_local and selectedpoints_local['range']:
        ...
        selection_bounds = {'x0': np.min(_df[x_col]), 'x1': np.max(_df[x_col]),
                            'y0': np.min(_df[y_col]), 'y1': np.max(_df[y_col])}
    ...
    # for an explanation
    fig = px.scatter(_df, x=_df[x_col], y=_df[y_col], text=_df.index)
